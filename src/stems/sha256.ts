...
export class IncrementalSha256 {
  ...
  update(input: ArrayBuffer | ArrayBufferView): this {
    ...
    if (this.#blockBytes !== 0) {
      const take = Math.min(64 - this.#blockBytes, bytes.byteLength);
      this.#block.set(bytes.subarray(0, take), this.#blockBytes);
    ...
    if (offset < bytes.byteLength) {
      ...
      this.#blockBytes = bytes.byteLength - offset;
    }
    return this;
  }
...
export async function deadline<T>(
  ...
  const interruption = new Promise<never>((_resolve, reject) => {
    timer = setTimeout(() => reject(new DOMException("Stem read deadline exceeded", "TimeoutError")), milliseconds);
    if (signal !== undefined) {
      abort = () => reject(signal.reason ?? new DOMException("Stem operation aborted", "AbortError"));
